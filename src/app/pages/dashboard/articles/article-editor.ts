...
import { ApiArticle, ApiChapter, BaseThemeSchema } from '../../../services/types';

@Component({
  selector: 'app-article-editor',
  standalone: true,
  imports: [CommonModule, FormsModule, CardModule, InputTextModule, TextareaModule, ButtonModule, SelectModule, CheckboxModule, ToastModule],
...
export class ArticleEditor implements OnInit {
    ...
    this.articleService.getArticleDetail(articleId).subscribe({
      next: (response) => {
        ...
        if (response.code === 200 && response.data) {
          ...
          this.loadChaptersForTheme(this.getThemeIdFromChapter(response.data.chapter_id));
        }
      },
      error: (error) => {
        this.loading = false;
    ...
    this.articleService.createArticle(
      ...
      next: (response) => {
        ...
        if (response.code === 200 || response.code === 201) {
          ...
          this.router.navigate(['/dashboard/articles', response.data?.id, 'edit']);
        }
      },
      error: (error) => {
        this.saving = false;
  ...
  updateArticle() {
    ...
    const updateData = {
      ...
      s3_content: this.article['content'] || '',
      chapter_id: this.article.chapter_id!,
      order: this.article.order || 0,
      is_active: this.article.is_active || false
    };

    this.articleService.updateArticleContent(updateData).subscribe({
      next: (response) => {
        this.saving = false;

...
import { ApiChapter, BaseThemeSchema } from '../../../services/types';

@Component({
  selector: 'app-chapters',
  standalone: true,
  imports: [CommonModule, TableModule, ButtonModule, InputTextModule, DialogModule, FormsModule, ConfirmDialogModule, ToastModule, SelectModule, TagModule],
...
export class Chapters implements OnInit {
  ...
  currentTheme: BaseThemeSchema | null = null;
  ...
  loadChaptersByTheme(themeId: string) {
    this.loading = true;
    this.currentTheme = this.themes.find(t => t.id === themeId) || null;
  ...
  openNewDialog() {
    this.newChapter = { name: '', description: '', theme_id: this.selectedThemeId };
  ...
  saveChapter() {
    ...
    if (this.editingChapter) {
      ...
      this.chapterService.updateOne(updatedChapter).subscribe({
        next: (response) => {
          ...
          if (response.code === 200) {
            ...
            this.closeDialog();
            setTimeout(() => this.loadChaptersByTheme(this.selectedThemeId), 100);
          }
        },
        error: (error) => {
          this.saving = false;
      ...
      // 创建新章节
      this.chapterService.createOne(this.newChapter.name, this.newChapter.theme_id).subscribe({
        next: (response) => {
          this.saving = false;
  ...
  get filteredChapters() {
    ...
    return this.chapters.filter(chapter => 
      ...
      (chapter.description && chapter.description.toLowerCase().includes(this.searchText.toLowerCase()))
    );
  }
